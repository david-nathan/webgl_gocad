...
angular.module("webglApp")
    .directive("vectorfieldCanvas", function () {
    function link(scope, element, attrs) {
        (function animation() {
            // vector field data
            var gamma = 2, c = 10, dt = 0.005, X0 = [], Y0 = [], X = [], Y = [], xb = 1.5, yb = 1.3;
            //// curve ////
            var N = 50, 
            ...
            var canvas = d3.select(element[0]).append("canvas").attr("width", windowWidth).attr("height", windowHeight);
            //// frame setup
            var width = 960, height = 500, mw = 0, g = canvas.node().getContext("2d"); // initialize a "canvas" element
            ...
            var xMap = d3.scale.linear()
                .domain([-xb, xb])
                .range([mw, width - mw]), yMap = d3.scale.linear()
                .domain([-yb, yb])
                .range([height - mw, mw]);
            //// animation setup
            var animAge = 0, frameRate = 30, M = X.length, MaxAge = 100, age = [];
            for (var i = 0; i < M; i++) {
                age.push(randage());
            }
            var drawFlag = true;
            // setInterval(function () {if (drawFlag) {draw();}}, frameRate);
            d3.timer(function () { if (drawFlag) {
                draw();
            } }, frameRate);
            d3.select(element[0]).on("click", function () { drawFlag = (drawFlag) ? false : true; });
            function randage() {
    ...
    return {
        ...
        restrict: 'E',
        replace: false,
        templateUrl: 'app/components/directives/vectorfieldcanvas/vectorfieldcanvas_template.html',
        scope: {}
    };
});
//# sourceMappingURL=vectorfieldcanvas.js.map